...
archivo_inventarios="/Users/santiago/Documents/SOLUMASTER/PROGRAMA_INVENTARIOS/Producto final/Excel enviar.xlsx"

#Se lee el archivo
#SIEMPRE SE DEBERÁ EDITAR EL NOMBRE DE LA HOJA SI CAMBIA 
df_exportar = pd.read_excel(archivo_inventarios, sheet_name='Hoja1',skiprows=2,skipfooter=1)
...
df_exportar=df_exportar.drop_duplicates('N° de Ìtem        ')
...
df = pd.read_excel(archivo_inventarios, sheet_name='Hoja1',skiprows=2,skipfooter=1)
#print(df.head())
df_creacion_vector_cantidades=df[['N° de Ìtem        ','Cant. uso           ']]
#se transforman a enteros para que coincidan con la lectura del codigo de barras.

df_creacion_vector_cantidades['N° de Ìtem        '] = df_creacion_vector_cantidades['N° de Ìtem        '].astype(int)
...
df=df[['N° de Ìtem        ','Descrip. art.                 ']]
#se transforman a enteros para que coincidan con la lectura del codigo de barras.
df['N° de Ìtem        '] = df['N° de Ìtem        '].astype(int)
...
def crear_excel():
    ...
    escrito = pd.ExcelWriter('Inventario_Solumaster_enviar.xlsx')
    # escribir el DataFrame en excel
    df_exportar.to_excel(escrito)
    # guardar el excel
    escrito.save()
    print('El DataFrame se ha escrito con éxito en el archivo de Excel.')


#Creación botón para crear excel
boton_excel= tk.Button(ventana, text="Crear Excel", command=crear_excel)
#boton_excel.place(x=100, y=200) 
boton_excel.pack(side = tk.BOTTOM)
    ...
    if tecla== kb.Key.enter:
        if bandera_enter==0:
            ...
            if it_is == True:
                ...
                label_producto.pack()
                #label codigo leido
                label_cantidad= tk.Label(text="Cantidad en 'KG' de la tinta", font="Helvetica 25")
                label_cantidad.pack()
                cantidad=tk.Entry(ventana, font="helvetica 25")
                cantidad.pack()
                boton_ingreso= tk.Button(ventana, text="Agregar al excel", command=habilitar_cantidad)
                boton_ingreso.pack()
                cantidad.focus()
            else:
                bandera_enter=0
